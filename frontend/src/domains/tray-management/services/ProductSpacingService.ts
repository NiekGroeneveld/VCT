...
export class ProductSpacingService {
   ...
   */
  static calculateAdvancedSpacing(
    tray: Tray,
    ...
    console.log(
      `TrayProductManager.calculateAdvancedSpacing for ${products.length} products in tray ${tray.id}`
    );
    ...
    if (availableSpace < 0) {
      console.warn(
        `Products don't fit in tray width ${trayWidth}mm, using simple spacing`
      );
  ...
  /**
   * Apply spacing to all products in all trays
   */
  static spaceOutAllTrays(trays: Tray[]): Tray[] {
    return trays.map(tray => {
      if (!tray.products || tray.products.length === 0) {
        return tray;
      }

      const spacedProducts = this.calculateAdvancedSpacing(tray, tray.products);
      
      return {
        ...tray,
        products: spacedProducts
      };
    });
  }
}
